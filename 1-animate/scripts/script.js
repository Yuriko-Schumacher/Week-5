...
d3.csv("data/data.csv").then(function (swimData) {
	data = swimData;
	console.log(data[0]);
...
function populateEvents(events) {
	...
	d3.select("select").on("change", function (d) {
		...
		let filteredData = data.filter(
			(e) => e.event === d.currentTarget.value
		);
		draw(filteredData);
	});
}
...
function draw(data) {
	let circleSel = containerG.selectAll("circle").data(data, (d) => d.id);

	circleSel
		.enter()
		.append("circle")
		.attr("fill", (d) => (d.sex === "Male" ? "olive" : "steelblue"))
		.attr("r", 3)
		.attr("cx", 0)
		.attr("cy", (d) => scaleY(d.date))
		.transition()
		.duration((d) => scaleDuration(d.milliseconds)) // can be dependent on data
		.attr("cx", (d) => scaleX(d.milliseconds));

	circleSel.exit().transition().duration(500).attr("opacity", 0).remove();
}
